...
## 1. 아키텍처 개요
본 인프라는 AWS 환경에서 최소한의 비용으로 마이크로서비스를 안정적으로 운영하기 위해 **3노드 통합 구조**를 채택합니다.
...
## 3. 핵심 기술 및 비용 최적화 전략
- **통합 컨트롤 플레인**: 관리 서비스(Consul)와 데이터 저장소를 하나의 노드에 통합하여 인스턴스 유지 비용 최소화.
- **Consul 도입**: 가벼운 Go 기반의 Consul을 통해 서비스 탐색과 설정을 관리하여 JVM 메모리 낭비 제거.
