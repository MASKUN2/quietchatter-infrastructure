...
- **변수 이스케이프**: Alloy 설정 등에서 `$1`을 사용할 때 테라폼 템플릿에서 과도하게 이스케이프(`$$1`)하지 않도록 주의 (실제 파일 생성 결과 확인 필수)
...
- **Secret 주입**: 모든 민감 정보는 AWS Secrets Manager에 등록 후 `user_data`에서 부팅 시 조회하여 환경 변수로 주입
