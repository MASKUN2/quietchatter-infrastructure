...
aws s3 cp "s3://$S3_BUCKET/$S3_PATH_PREFIX/config/docker-compose.yaml" \
...
aws s3 cp "s3://$S3_BUCKET/$S3_PATH_PREFIX/config/config.alloy" \
...
# 4. nginx.conf 동기화 (존재하는 경우만)
log "INFO: nginx.conf 동기화 시도 중..."
if aws s3 ls "s3://$S3_BUCKET/$S3_PATH_PREFIX/config/nginx.conf" --region "$AWS_REGION" > /dev/null 2>&1; then
  aws s3 cp "s3://$S3_BUCKET/$S3_PATH_PREFIX/config/nginx.conf" \
    /tmp/nginx.conf.new --region "$AWS_REGION"

  if [ ! -f /data/app/nginx.conf ] || \
     ! diff -q /tmp/nginx.conf.new /data/app/nginx.conf > /dev/null 2>&1; then
    cp /tmp/nginx.conf.new /data/app/nginx.conf
    NGINX_CHANGED=true
    log "INFO: nginx.conf가 변경되었습니다."
  fi
else
  log "INFO: nginx.conf가 S3에 없어 건너뜁니다."
fi

# 5. 변경 반영
if [ "$COMPOSE_CHANGED" = true ] || [ "$SECRETS_CHANGED" = true ] || [ "$NGINX_CHANGED" = true ]; then
