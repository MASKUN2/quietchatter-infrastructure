...
노드 그룹 구성
- NAT 및 Ingress 노드: t4g.nano (0.5GB), NAT 및 NGINX 구동, 외부 게이트웨이 및 라우팅 역할 수행
- Controlplane 노드: t4g.small (2.0GB), Postgres, Redis, Kafka, Consul 구동, 시스템 상태 및 데이터 관리 핵심부
- Application 노드: t4g.micro (1.0GB), Spring Cloud Gateway 및 마이크로서비스 구동, 서비스 로직 실행부
- Frontend 노드: t4g.micro (1.0GB), Next.js BFF 및 Consul 클라이언트 에이전트 구동, 외부 사용자 요청 수신 및 내부 서비스 라우팅
...
예상 비용
- 서울 리전 기준 월 총합 약 32.00 달러
- 초기 설계 대비 약 58퍼센트 비용 절감 달성
...
- 계층 실행 순서: 01-base → 02-ingress → 03-platform → 04-apps-gateway → 05-apps-microservices → 06-apps-frontend
...
고가용성 및 확장성
- API Gateway ASG 도입: 현재 단일 EC2를 Auto Scaling Group으로 전환하고 ALB를 통해 트래픽 분산
...
보안 강화
- SSL/TLS 적용: ALB 도입 시 ACM을 통해 HTTPS 강제
...
완료된 태스크 (Frontend Consul DNS 연동)
- 완료: frontend 서비스가 Consul에 등록되었으며, API Gateway 호출이 고정 IP 대신 Consul DNS(microservice-api-gateway.service.consul)로 변경됨
- 06-apps-frontend 레이어가 신설되어 Next.js BFF 노드와 consul-agent를 독립 계층으로 관리
...
- 도입 구조: 06-apps-frontend 레이어를 신설하고 t4g.micro 노드에 consul-agent와 Next.js BFF 컨테이너를 배치. INTERNAL_API_GATEWAY_URL을 http://microservice-api-gateway.service.consul:8080으로 설정하여 Consul DNS를 통한 동적 라우팅 적용
