# Gemini CLI - QuietChatter Project Entry

이 프로젝트에서 Gemini CLI를 사용할 때의 필수 진입 지침입니다.

## 1. 초기화 지침 (Mandatory)
- 모든 세션 시작 시, 프로젝트 루트의 GEMINI.md를 가장 먼저 읽고 그 지시에 따르십시오.
- GEMINI.md는 이 프로젝트의 모든 아키텍처 원칙, 행동 규정 및 정보 습득 순서를 정의하는 단일 출처(SSOT)입니다.
- 모든 작업 시 superpowers 스킬들을 확인하고 필요시 활성화하여 사용하십시오.

## 2. 참조 순서 준수
- GEMINI.md에 명시된 순서(GEMINI.md -> docs/requirements/prd.md -> docs/ARCHITECTURE.md 등)대로 문맥을 파악한 후 작업을 시작하십시오.
- 모든 코딩 스타일과 소통 방식(강조 서식 및 이모티콘 사용 금지 등)은 GEMINI.md의 규정을 엄격히 준수하십시오.
# AI Agent Guide - Infrastructure Module

이 문서는 AI 에이전트가 QuietChatter 프로젝트를 이해하고 관리하기 위한 핵심 지침서입니다.

## 1. 정보 습득 순서 (Mandatory)

모든 작업 시작 전 및 작업 중에 superpowers 스킬 목록을 항상 확인하고 상황에 맞는 스킬을 활성화하여 사용하십시오.

작업을 시작하기 전, 에이전트는 반드시 다음 순서로 프로젝트 문맥을 파악해야 합니다.
1. AGENTS.md (현재 문서): 아키텍처 원칙 및 작업 규정 확인
...
- 강조 서식 제한: 마크다운 작성 시 굵게(bold)나 기울임(italics) 서식을 절대 사용하지 않습니다.
- 이모티콘 금지: 어떠한 상황에서도 이모티콘을 사용하지 않습니다.
- 문서 업데이트: 작업 완료 후 관련 내용이 가이드나 로그에 반영되어야 한다면 사용자에게 업데이트를 제안합니다.

## 4. 주요 기술적 교훈 (Lessons Learned)
